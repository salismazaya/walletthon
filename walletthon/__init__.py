...
from .types import OwnOffer, Offer
...
class Wallet:
    ...
    def get_auth_token(self) -> str:
        ...
        keren = messages.RequestWebViewRequest(
            get_input_entity('@wallet'), get_input_entity('@wallet'), 'Android'
        )
        url = self.client(keren).url
        regex_pattern = r'tgWebAppData=([^&]+)'
        match = re.search(regex_pattern, url)
        tgWebAppData_value = unquote(match.group(1))

        data = {
            "web_view_init_data_raw": tgWebAppData_value,
        }

        response = self.session.post('https://walletbot.me/api/v1/users/auth/', json = data).json()
